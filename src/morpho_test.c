...
fusion_swp8:
    ...
    //ouverture3_ui8matrix_swp8_basic                      (X8, 0, h-1, 0, w8-1, T_basic, Y8_basic                      );
...
free:
    ...
    zero_ui8matrix(Y1_basic                      , 0, h-1, 0, w1-1);
    zero_ui8matrix(Y1_fusion_basic               , 0, h-1, 0, w1-1);
    zero_ui8matrix(Y1_fusion_red                 , 0, h-1, 0, w1-1);
    zero_ui8matrix(Y1_fusion_ilu3_red            , 0, h-1, 0, w1-1);
    zero_ui8matrix(Y1_fusion_ilu3_elu2_red       , 0, h-1, 0, w1-1);
    zero_ui8matrix(Y1_fusion_ilu3_elu2_red_factor, 0, h-1, 0, w1-1);
...
free:
    ...
    free_ui8matrix(T64_pipeline_basic               , 0 - r, h-1 + r, 0 - r, w64-1 + r);
    free_ui8matrix(T64_pipeline_red                 , 0 - r, h-1 + r, 0 - r, w64-1 + r);
    free_ui8matrix(T64_pipeline_ilu3_red            , 0 - r, h-1 + r, 0 - r, w64-1 + r);
    free_ui8matrix(T64_pipeline_elu2_red            , 0 - r, h-1 + r, 0 - r, w64-1 + r);
    free_ui8matrix(T64_pipeline_elu2_red_factor     , 0 - r, h-1 + r, 0 - r, w64-1 + r);
    free_ui8matrix(T64_pipeline_ilu3_elu2_red       , 0 - r, h-1 + r, 0 - r, w64-1 + r);
    free_ui8matrix(T64_pipeline_ilu3_elu2_red_factor, 0 - r, h-1 + r, 0 - r, w64-1 + r);
...
// -----------------------------
{
    ...
    for(int h = h0; h <= h0+dh; h++) {
        for(int w = w0; w <= w0+dw; w+= sw) {
            //s += test_morpho_ouverture_routine_SWP8(h, w);
            s += test_morpho_ouverture_routine_SWP64(h, w);
        }
    }
